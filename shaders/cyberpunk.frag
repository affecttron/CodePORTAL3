...
// in-game post-FX: CA, scanlines, neon grade, vignette, pulsed glitch
...
void main() {
    ...
    if (burst > 0.01) {
        ...
        float bandRand = hash(vec2(band, 17.0));
    ...
    float ca       = (0.0006 + 0.0018 * dist) * fx + burst * 0.004;
    ...
    col *= mix(1.0, 0.93 + 0.07 * scan, fx);

    // neon glow on bright pixels
    float lum = dot(col, vec3(0.299, 0.587, 0.114));
    col += col * smoothstep(0.65, 1.0, lum) * 0.18 * fx;
    ...
    // vignette
    float vig = smoothstep(1.10, 0.55, dist);
    col *= mix(1.0, vig, fx * 0.35);

    // grain
    float grain = (hash(v_uv * u_resolution + u_time * 60.0) - 0.5) * 0.025 * fx;
    col += grain;
